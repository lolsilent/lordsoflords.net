...
print '
<table width="100%"><tr><th>'.$title.' graveyard</th></tr>
';
if($presult=mysqli_query($link, "SELECT * FROM `$tbl_papers` WHERE (`server_id`='$row->server_id' and `pid`='3') ORDER BY `id` DESC LIMIT 50")){
...
if($lpresult=mysqli_query($link, "SELECT * FROM `$tbl_papers` WHERE (`server_id`='$row->server_id' and `pid`='3' and `timer`<($current_time-(60*60*24*5))) ORDER BY `id` DESC")){
...
mysqli_query($link, "DELETE FROM `$tbl_papers` WHERE (`server_id`='$row->server_id' and `pid`='3' and `timer`<($current_time-(60*60*24*30)))");
/*
$maxfilesize=50000;

$filename='graves/'.$logdate.'.php';
