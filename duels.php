...
$row->server_id=clean_post($_GET['served']);
	...
	$row->server_id=1;}}
print '
<table width="100%"><tr><th>'.$title.' duels</th></tr>
';
if($presult=mysqli_query($link, "SELECT * FROM `$tbl_papers` WHERE (`server_id`='$row->server_id' and `pid`='2') ORDER BY `id` DESC LIMIT 50")){
...
if($lpresult=mysqli_query($link, "SELECT * FROM `$tbl_papers` WHERE (`server_id`='$row->server_id' and `pid`='2') ORDER BY `id` DESC")){
...
mysqli_query($link, "DELETE FROM `$tbl_papers` WHERE (`server_id`='$row->server_id' and `pid`='2' and `timer`<($current_time-(60*60*24*7)))");
/*
$maxfilesize=50000;

$filename='duels/'.$logdate.'.php';
